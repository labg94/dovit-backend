...
public class LicenseServiceImpl implements LicenseService {
    ...
    public List<LicenseDTO> findAllByToolId(Long toolId) {
        ...
        return licenses.stream().map(l -> {
            ...
            licenseDTO.setLicensePricing(l.getPricings().stream().map(p -> new LicensePricingResponse(p.getId(), p.getMinUsers(), p.getMaxUsers(), p.getPrice())).collect(Collectors.toList()));
            return licenseDTO;
